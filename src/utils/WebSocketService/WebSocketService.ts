...
export default class WebSocketService {
  ...
  private _newMessangeSoket(chatId:number) {
    ...
    const index = state.chats.findIndex((item:ChatType) => item.id === chatId);
    ...
    let newMessages;
    const oldMessages = state.chats[index].messages ? state.chats[index].messages : [];
  ...
  private _closeSoket(chatId:number) {
    ...
    if (eventMessage.wasClean) {
      ...
      if (eventMessage.code === 1006) {
        const indexSocket = this._sockets.findIndex((item) => item.chatId === chatId);
  ...
  public send(chatId:number, messange:sendData) {
    const indexChat = this._sockets.findIndex((item) => item.chatId === chatId);
